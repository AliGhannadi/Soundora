...
class StatsAdmin(admin.ModelAdmin):
    change_list_template = (
        "admin/stats_change_list.html"  # custom template for page button
    )
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                "report/",
                self.admin_site.admin_view(self.stats_report),
                name="stats-report",
            ),
        ]
    ...
    def stats_report(self, request):
        ...
        total = (
            stats.win + stats.mac + stats.iphone + stats.android + stats.other
        ) or 1
        data = [
            {
                "name": "Windows",
