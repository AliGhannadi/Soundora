...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for _ in range(5):
         user = User.objects.create_user(email=self.fake.email(), password="Test123456", username=self.fake.user_name(), phone_number=self.fake_iran_phone())
        ...
        for _ in range(10):
            category = Category.objects.create(name=self.fake.sentence(nb_words=1))
            music =  Music.objects.create(title=self.fake.sentence(nb_words=1), category=category, file="musics/AdeleHello.mp3/")
