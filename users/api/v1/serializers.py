...
class SMSVerificationSerializer(serializers.Serializer):
    ...
    #      raise serializers.ValidationError({"detail": "Limitation reached. Plaese try again later."})
     if cached_code is None:
        raise serializers.ValidationError({"detail": "code is expired or not found or user is already verified."})
     if str(cached_code) != str(code):
        raise serializers.ValidationError({"detail": "Invalid verification code. try again."})
     try:
